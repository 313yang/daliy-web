...
import { getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, signInWithRedirect, GithubAuthProvider, GoogleAuthProvider, signInWithPopup } from "firebase/auth";
      ...
      if (type === "join") {
        ...
        signInWithEmailAndPassword(authService, email, password)
          ...
          .catch((error) => {
            ...
            else if (errorCode === "auth/wrong-password") setLoginError("비밀번호가 일치하지 않아요");
          });
      }
    } catch (error) {
  ...
  const onSocialLogin = async ({ target }) => {
    ...
    signInWithPopup(authService, name === "google" ? googleProvider : githubProvider)
      .then((result) => {
        ...
        const credential = name === "google" ? GoogleAuthProvider.credentialFromResult(result) : GithubAuthProvider.credentialFromResult(result);
    ...
    <Container>
      ...
      <Form onSubmit={handleOnSubmit}>
        <Label style={{ letterSpacing: 1.5 }}>
          이메일 &nbsp;:
          <Input name="email" type="email" placeholder="xxx@xxx.xxx" required value={email} onChange={handleOnChange} />
        </Label>
        <Label>
          비밀번호 :
          <Input name="password" type="password" placeholder="비밀번호를 입력해주세요." required value={password} onChange={handleOnChange} />
        </Label>
        {type === "join" && (
          <Label>
            비번확인 :
            <Input name="passwordConfirm" type="password" placeholder="비밀번호를 입력해주세요" required value={passwordConfirm} onChange={handleOnChange} />
          </Label>
        )}
        {loginError && <p>{loginError}</p>}
        {type === "join" && password.length > 0 && passwordConfirm.length > 0 && password !== passwordConfirm && <p>비밀번호가 일치하지 않아요!</p>}
      ...
      {type === "login" ? <LinkButton onClick={() => navigate("/auth/join")}>아직 회원이 아니신가요?</LinkButton> : <LinkButton onClick={() => navigate("/auth/login")}>이미 회원이신가요?</LinkButton>}
    </Container>
  );
};
export default Auth;
